                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded mt-1 inline-block ${isDebt ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-500'}`}>
                            {isDebt ? 'Liabilitas' : acc.type.replace('-', ' ')}
                          </span>
                        </div>
                      </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <select value={type} onChange={(e) => setType(e.target.value)} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl bg-white text-sm outline-none focus:ring-2 focus:ring-slate-900" disabled={addAccountMutation.isPending}>
            ...
            <div>
              ...
              <input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm outline-none focus:ring-2 focus:ring-slate-900" placeholder="Contoh: BCA Utama" disabled={addAccountMutation.isPending} />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1 uppercase tracking-wider">Saldo Awal (Rp)</label>
              <input type="number" required value={balance} onChange={(e) => setBalance(e.target.value)} className={`w-full px-4 py-2.5 border rounded-xl text-sm outline-none transition-colors ${type === 'debt' ? 'border-red-300 focus:ring-red-600 bg-red-50/30' : 'border-slate-300 focus:ring-2 focus:ring-slate-900'}`} placeholder="0" disabled={addAccountMutation.isPending} />
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden flex flex-col animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleInvite} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1 uppercase tracking-wider">Email Partner</label>
                  <input type="email" required value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-600" placeholder="contoh: pasangan@email.com" disabled={isInviting} />
